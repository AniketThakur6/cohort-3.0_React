...
const ProductCard = ({ product, inCart }) => {
  let { setCartItems,incrementCart,decrementCart } = useContext(MyStore);

  const addToCart = () => {
    setCartItems((prev) => [...prev, { ...product, quantity: 1 }]);
    alert("Items is added in Cart");
  };
    ...
    <div className="h-110 text-lg flex flex-col w-65 justify-between rounded-lg  p-3 bg-zinc-800">
      <div className="flex flex-col gap-2 h-60">
        <div className="flex justify-center">
          <img className="h-40 object-fit" src={product.image} alt="" />
        </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div>
          ...
          {inCart? <button className="w-full h-11 bg-blue-900 rounded-xl text-2xl flex justify-center items-center gap-4">
            ...
            onClick={addToCart}
            className="w-full h-11 bg-blue-900 rounded-xl"
          >
            Add to Cart
          </button> }
